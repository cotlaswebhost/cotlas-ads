...
final class Cotlas_Ads_Admin {
	...
	private function injection_page(): void {
		...
		?><form class="cotlas-card" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><?php wp_nonce_field('cotlas_save_injections'); ?><input type="hidden" name="action" value="cotlas_save_injections"><div class="card-title"><div><h2>Automatic placement injection</h2><p class="muted">Create as many independent rules as needed. Interstitial and sticky placement types are automatic and do not need rules here.</p></div><button type="button" class="button" data-add-injection>Add rule</button></div><div data-injection-rules><?php foreach ($rules as $index => $rule) $this->injection_rule_row($index, $rule, $zones); ?></div><button class="cotlas-primary">Save injection rules</button></form><?php
	}
	...
	private function injection_rule_row(int $index, array $rule, array $zones): void {
		...
		?><div class="cotlas-injection-rule" data-injection-rule><div class="field-row"><label>Placement<select name="injection_rules[<?php echo $index; ?>][zone]"><option value="">Disabled</option><?php foreach ($zones as $zone): ?><option value="<?php echo esc_attr($zone['slug']); ?>" <?php selected($rule['zone'], $zone['slug']); ?>><?php echo esc_html($zone['name']); ?></option><?php endforeach; ?></select></label><label>Location<select name="injection_rules[<?php echo $index; ?>][location]"><?php foreach (array('before' => 'Before post content', 'after' => 'After post content', 'paragraph' => 'After paragraph number', 'feed' => 'Before archive/feed item number', 'header' => 'Visible header / after body opens', 'footer' => 'Footer', 'sidebar_before' => 'Before first sidebar widget', 'sidebar_after' => 'After all sidebar widgets') as $value => $label): ?><option value="<?php echo esc_attr($value); ?>" <?php selected($rule['location'], $value); ?>><?php echo esc_html($label); ?></option><?php endforeach; ?></select></label></div><div class="field-row"><label>Paragraph or feed item number<input type="number" min="1" max="100" name="injection_rules[<?php echo $index; ?>][number]" value="<?php echo absint($rule['number']); ?>"><small>Used only for paragraph and archive/feed rules.</small></label><label>Post types<input name="injection_rules[<?php echo $index; ?>][post_types]" value="<?php echo esc_attr(is_array($rule['post_types']) ? implode(',', $rule['post_types']) : $rule['post_types']); ?>" placeholder="post,page,product"><small>Comma-separated; used for content rules.</small></label></div><label>Sidebar ID (optional)<input name="injection_rules[<?php echo $index; ?>][sidebar_id]" value="<?php echo esc_attr($rule['sidebar_id']); ?>" placeholder="sidebar-1"><small>Leave blank for every sidebar, or enter the registered sidebar ID.</small></label><button type="button" class="button-link-delete" data-remove-injection>Remove rule</button></div><?php
	}
