...
final class Cotlas_Ads_Admin {
	...
	private function ad_form(?array $ad): void {
		$ad = wp_parse_args($ad ?: array(), array('id' => 0, 'name' => '', 'status' => 'draft', 'creative_type' => 'html', 'content' => '', 'image_id' => 0, 'slider_image_ids' => '', 'target_url' => '', 'canvas_width' => 0, 'canvas_height' => 0, 'slider_interval' => 5, 'weight' => 10, 'start_at' => '', 'end_at' => '', 'days' => '', 'hours' => '', 'device' => 'all', 'countries' => '', 'include_logged_in' => 1, 'max_impressions' => 0, 'max_clicks' => 0));
		...
		<form class="cotlas-editor" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
			...
			<div class="cotlas-card editor-main"><div class="card-title"><h2><?php echo $ad['id'] ? 'Edit campaign' : 'New campaign'; ?></h2><a href="<?php echo esc_url($this->url('ads')); ?>">Cancel</a></div>
				...
				<div class="field-row"><label>Creative type<select name="creative_type" data-creative-type><option value="html" <?php selected($ad['creative_type'], 'html'); ?>>HTML / ad tag</option><option value="image" <?php selected($ad['creative_type'], 'image'); ?>>Single image</option><option value="slider" <?php selected($ad['creative_type'], 'slider'); ?>>Image slider</option></select><small>Choose one image, a rotating image slider, or custom HTML from an ad network.</small></label><label>Destination URL<input type="text" name="target_url" value="<?php echo esc_attr($ad['target_url']); ?>" placeholder="https://advertiser.example/ or tel:+91..."><small>Optional link opened when a reader clicks the creative.</small></label></div>
				...
				<div data-creative-panel="image"><label>Campaign image<input type="hidden" name="image_id" value="<?php echo absint($ad['image_id']); ?>" data-media-id><button type="button" class="button" data-media-single>Select image</button><small>Select an image from the WordPress Media Library. Its preview appears below.</small><div class="media-preview" data-media-preview><?php if ($image_url): ?><img src="<?php echo esc_url($image_url); ?>" alt=""><?php endif; ?></div></label></div>
				<div data-creative-panel="slider"><label>Slider images<input type="hidden" name="slider_image_ids" value="<?php echo esc_attr(implode(',', $slider_ids)); ?>" data-slider-ids><button type="button" class="button" data-media-slider>Select multiple images</button><small class="field-warning">Upload images with exactly the same dimensions. For example, use four 728×90 images for a desktop campaign or four 300×250 images for a mobile campaign.</small><div class="media-preview slider-preview" data-slider-preview><?php foreach ($slider_ids as $slider_id): echo wp_get_attachment_image($slider_id, 'thumbnail'); endforeach; ?></div></label><label>Slide interval (seconds)<input type="number" min="2" max="60" name="slider_interval" value="<?php echo absint($ad['slider_interval']); ?>"><small>How long each image remains visible before the next slide.</small></label></div>
			...
			<aside class="cotlas-card editor-side"><h2>Delivery</h2><label>Status<select name="status"><option value="active" <?php selected($ad['status'], 'active'); ?>>Active</option><option value="paused" <?php selected($ad['status'], 'paused'); ?>>Paused</option><option value="draft" <?php selected($ad['status'], 'draft'); ?>>Draft</option></select><small>Active campaigns still obey dates, caps, device, country, weekday, and hour rules. Expired and Scheduled are calculated automatically.</small></label>
				<fieldset><legend>Placements</legend><small>Select every placement where this campaign may appear. You can also manage the same assignment from Placements.</small><?php foreach ($this->repository->zones() as $zone): ?><label class="check"><input type="checkbox" name="zone_ids[]" value="<?php echo absint($zone['id']); ?>" <?php checked(in_array((int) $zone['id'], $zone_ids, true)); ?>><?php echo esc_html($zone['name']); ?></label><?php endforeach; ?></fieldset>
	...
	private function zones_page(): void {
		...
		<label>Name<input required name="name" value="<?php echo esc_attr($zone['name']); ?>"><small>A dashboard label for this reusable ad location.</small></label><label>Slug<input name="slug" value="<?php echo esc_attr($zone['slug']); ?>" placeholder="article-inline"><small>Used in the shortcode and PHP function. Keep it stable after adding it to a template.</small></label><label>Delivery<select name="mode"><option value="weighted" <?php selected($zone['mode'], 'weighted'); ?>>Weighted rotation</option><option value="random" <?php selected($zone['mode'], 'random'); ?>>Equal random</option><option value="all" <?php selected($zone['mode'], 'all'); ?>>Show all</option></select><small>Weighted uses campaign weights; random gives equal chances; show all prints every eligible campaign.</small></label>
		<fieldset><legend>Campaigns</legend><small>Only eligible checked campaigns can display here. Assignments can also be made from the campaign editor.</small><?php foreach ($this->repository->ads() as $ad): ?><label class="check"><input type="checkbox" name="ad_ids[]" value="<?php echo absint($ad['id']); ?>" <?php checked(in_array((int) $ad['id'], $selected, true)); ?>><?php echo esc_html($ad['name']); ?></label><?php endforeach; ?></fieldset><label>CSS class<input name="css_class" value="<?php echo esc_attr($zone['css_class']); ?>"><small>Optional class added to the placement wrapper for theme-specific styling.</small></label><label>Fallback HTML<textarea name="fallback" rows="4"><?php echo esc_textarea($zone['fallback']); ?></textarea><small>Displayed only when every assigned campaign is paused, scheduled, expired, capped, or otherwise ineligible.</small></label><button class="cotlas-primary">Save placement</button></form></div>
		<?php
	}
	...
	public function save_ad(): void { $this->guard('cotlas_ads_manage', 'cotlas_save_ad'); $id = $this->repository->save_ad(wp_unslash($_POST)); $this->repository->assign_ad_to_zones($id, (array) ($_POST['zone_ids'] ?? array())); $this->redirect('ads'); }
